...
class Banggood {
    ...
    public function getProduct($product_id){
        ...
        $data = array(
            ...
            'price' => ((float) $price_pp + $shipFee),
            'oldPrice' => false,
            'thumb' => $prod['product_main_image_url'] == null ? $product_bg['img'] : $prod['product_main_image_url'],
            'image' => $this->images($res['image_list']['large']),
            'description' => html_entity_decode(utf8_encode($res['description'])),
            'rating' => 0,
            'quantity' => $stock_gps,
            'tag' => 'test',
            'viewed' => 0,
            'totalSold'=> '10 sold',
            'warehouse' => $this->getStock($res['poa_list'], $result_pp, $result_gps, $product_id, $shipFee, $product_bg['img']),
            ...
            'detail'  => array(
                array(
                  ...
                  array(    
                    'label' => 'Shipping',
                    'value' => '₱'.$result_gs['shipmethod_list'][0]['shipfee']
                ), 
                  array(
                    'label' => 'Shipping Time',
                    'value' => $result_gs['shipmethod_list'][0]['shipday'] .' business days'
                )
            ),
            'attribute' => [],
                    ...
                    foreach($poa_list as $key => $value){
                            ...
                            foreach($value['option_values'] as $list){
                                $data[$key]['value'][] = array(
                                        ...
                                        'img' => $list['view_image'] == null ? $defaultImage : $list['view_image'],
    ...
    public function stockAndPriceFilter($prices, $stocks, $values, $product_id, $shipFee){
        ...
        foreach($stocks['stocks'][0]['stock_list'] as $stock){
            ...
            $data[] = array(
                ...
                'stock' => $stock['stock'],
                'msg' => $stock['stocks'][0]['stock_list'][$i]['stock_msg'],
                'selected' => false,
                'key' => $key
            );
            $i++;
