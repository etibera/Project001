...
class Checkout {
    ...
    public function checkoutDetails($product_ids, $customer_id, $total_price){
        ...
        $data = array();
        $addresses = $address->getAddresses($customer_id);
        ...
        $data['shippingRate'] = $this->shippingRateData($product_ids, $customer_id, $total_price);
        $data['text_payment'] = 'Your order will not ship until we receive payment.';
        $data['bank'] = "account number : 003610116131 <br> naccount name: Accenthub, Inc.<br>Bank Name : BDO unibank<br>";
        return $data;
    }
    public function shippingRateData($product_ids, $customer_id, $total_price){
        $codValidate = $this->validateAmount($total_price);
		$sr = array();
		$sr[] = array(
			'name' => 'Flat Shipping Rate',
			'total' => $this->flat($product_ids, $customer_id),
			'value' => 'flat.flat',
			'disable' => false,
		...
		$pm[] = array(
			...
			'value' => 'cod',
			'disable' => $codValidate == "" ? false : true,
			'message' => $codValidate
		);
		$pm[] = array(
    ...
    public function validateAmount($total_price){
        ...
        return $total > 5000.00 ? "It can't be used when your purchase is more than 5,000" : "";
    }
             ...
             if($product_total<=$querymax_qty){
                ...
                if($modulo==0){
                    ...
                    if($boxQTY<$querymax_qty_con){
                        ...
                        $select_convert->execute();
                        $queryconvert=$select_convert->fetch(PDO::FETCH_ASSOC);
                        $convert_amount=$queryamount['amount'];
    ...
    public function addOrder($token){
        ...
        try{
            ...
            if($payment_method['value'] == 'bank_transfer'){
                $comment  = "account number : 003610116131\r\naccount name: Accenthub, Inc.\r\nBank Name : BDO unibank\r\n";
            }
    ...
    public function insertOrderProduct($product_ids, $customer_id, $order_id, $token){
        ...
        foreach($this->getSelectedCartProduct($customer_id, $product_ids) as $product){
            if($product['p_type'] == 0){
                ...
                total = :total, tax = 0.0000, reward = 0, p_type = :p_type");
            ...
            if($product_info){
                switch($product_info['type']){
                    case 'reg':
                        ...
                        $product_quantity = $cart->product_quantity($product_info['product_id']);
                        $data[] = array(
