...
class Product {
    ...
    public function brandProducts($pageNum, $brandId){
        ...
        $st1 = $this->conn->prepare("SELECT p.product_id, pd.name, p.image, p.price, 'reg' as `type` FROM product_to_brand ptb 
        JOIN oc_product_brand pb ON pb.id = ptb.brand_id 
        JOIN oc_product p ON p.product_id = ptb.product_id
        JOIN oc_product_description pd ON pd.product_id = p.product_id
        WHERE pb.id = :brandId ORDER by p.date_added desc LIMIT :offset , :no_per_page");
    ...
    public function localProduct($product_id, $storeId){
       ...
       if($select_product->rowCount()){
            ...
            return array(
                ...
                'price' => $data['special'] != null ? $data['special'] : $data['price'],
       ...
       global $image;
        ...
        try {
            ...
            p.model,
             ...
             bg.status, bg.product_id as model, null as special, null as rating, 'bg' as `type` FROM bg_product bg
            --  UNION All
            --  SELECT cp.goods_sn as product_id, ROUND(((cp.price + IFNULL((SELECT rate FROM foreign_exchange_rates WHERE date_added = STR_TO_DATE(NOW(), '%Y-%m-%d')), (select IFNULL(rate, 0) from foreign_exchange_rates ORDER BY date_added DESC LIMIT 1))) * 1.3), 2) as price, cp.product_title as name, cp.product_img as image, 
            --  cp.status, cp.sku as model, null as special, null as rating, 'cb' as `type` FROM oc_china_product cp
             UNION All
             SELECT ap.id as product_id, ROUND(((ap.price + IFNULL((SELECT rate FROM foreign_exchange_rates WHERE date_added = STR_TO_DATE(NOW(), '%Y-%m-%d')), (select IFNULL(rate, 0) from foreign_exchange_rates ORDER BY date_added DESC LIMIT 1))) * 1.3), 2) as price, ap.name, ap.image, 
             ap.status, ap.product_id as model, null as special, null as rating, 'ae' as `type` FROM aliexpress_products ap
             ) as pp 
    ...
    public function get_category_products($data = array()){
        $sql = "SELECT p.product_id, (SELECT AVG(rating) AS total FROM oc_review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, 
        (SELECT price FROM oc_product_discount pd2 WHERE pd2.product_id = p.product_id 
         AND pd2.quantity = '1' AND ((pd2.date_start = null OR pd2.date_start < convert_tz(utc_timestamp(),'-08:00','+0:00')) 
         AND (pd2.date_end = null OR pd2.date_end > convert_tz(utc_timestamp(),'-08:00','+0:00'))) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, 
         (SELECT price FROM oc_product_special ps WHERE ps.product_id = p.product_id 
        AND ((ps.date_start = null OR ps.date_start < convert_tz(utc_timestamp(),'-08:00','+0:00')) AND (ps.date_end = null OR ps.date_end > convert_tz(utc_timestamp(),'-08:00','+0:00'))) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special";
    ...
    $sql .= " LEFT JOIN oc_product_special ps ON (ps.product_id = p.product_id)";
    ...
    WHERE p.status = '1' AND p.date_available <= convert_tz(utc_timestamp(),'-08:00','+0:00')";
        ...
        if(isset($data['price_range'])){
			if(count($data['price_range']) != 0){
				$sql .= " AND (CASE WHEN ps.price IS NOT NULL THEN ps.price ELSE p.price END) ";
		...
		foreach ($st->fetchAll() as $result) {
            // if($result !== false){
                $product_data[] = $this->get_single_product($result['product_id'], 'reg');
            // }
		}
    ...
    public function get_products($data = array()){
        $sql = "SELECT pp.product_id, pp.rating, pp.discount, pp.special, pp.type, pp.name FROM (
        SELECT 'bg' as `type`, bp.product_id, null as rating, null as discount, null as special,  bp.status, bp.add_date as date_added, bp.modify_date as date_modified, bp.product_name as name, null as special_price, bp.price  FROM bg_product bp WHERE bp.status = 1
        UNION ALL
           ";
        $sql .= "SELECT 'reg' as `type`, p.product_id,
        ...
        (SELECT price FROM oc_product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.quantity = '1' AND ((pd2.date_start = null OR pd2.date_start < convert_tz(utc_timestamp(),'-08:00','+0:00')) AND (pd2.date_end = null OR pd2.date_end > convert_tz(utc_timestamp(),'-08:00','+0:00'))) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, 
        (SELECT price FROM oc_product_special ps WHERE ps.product_id = p.product_id AND ((ps.date_start = null OR ps.date_start < convert_tz(utc_timestamp(),'-08:00','+0:00')) AND (ps.date_end = null OR ps.date_end > convert_tz(utc_timestamp(),'-08:00','+0:00'))) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special,";
        $sql .= "p.status,
        p.date_added,
        p.date_modified,
        pd.name, ps.price as special_price, p.price";
    //    if (!empty($data['filter_category_id'])) {
    //     if (!empty($data['filter_sub_category'])) {
    //         $sql .= " FROM oc_category_path cp LEFT JOIN oc_product_to_category p2c ON (cp.category_id = p2c.category_id)";
    //     } else {
    ...
    $sql .= " WHERE pp.status = '1' ";
        ...
        if(isset($data['price_range'])){
			if(count($data['price_range']) != 0){
				$sql .= " AND (CASE WHEN pp.special_price IS NOT NULL THEN pp.special_price ELSE pp.price END) ";
				if($data['price_range'][1] == 'Above'){
					$sql .= ">= ".$data['price_range'][0];
				}else{
					$sql .= "BETWEEN ".$data['price_range'][0]." AND ".$data['price_range'][1];
        ...
        $sql .= " GROUP BY pp.product_id, pp.rating, pp.discount, pp.special, pp.type, pp.name";
        ...
        if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			if ($data['sort'] == 'pp.name' || $data['sort'] == 'pp.model') {
				...
				$sql .= " ORDER BY (CASE WHEN special IS NOT NULL THEN special WHEN discount IS NOT NULL THEN discount ELSE pp.price END)";
			} else {
				$sql .= " ORDER BY " . $data['sort'];
			}
		} else {
		...
		foreach ($st->fetchAll() as $result) {
                // print_r($result);
            // if($result !== false){
                $product_data[] = $this->get_single_product($result['product_id'], $result['type']);
            // }
		}
		return $product_data;
        ...
        $sql = "SELECT p.*,pd.name, 'reg' as type FROM oc_product p 
  ...
  global $image;
    ...
    public function getDiscoverProducts($productIds, $page_number){
        ...
        $decodeProductIds = json_decode($productIds);
        if(count($decodeProductIds) > 0){
            $s = $this->conn->prepare("SELECT p2c.category_id, pd.name FROM oc_product_to_category p2c 
            LEFT JOIN oc_product_description pd ON pd.product_id = p2c.product_id WHERE pd.product_id IN(:product_id) ORDER BY FIELD(pd.product_id, :product_id)");
            $s->bindValue(":product_id", rtrim(implode(',', json_decode($productIds)), ","));
            $s->execute();
            foreach($s->fetchAll(PDO::FETCH_ASSOC) as $p1){
                $categoryIds .= $p1['category_id']. ',';
                $firstWordNamesArray[] = explode(' ', $p1['name'])[0];
            }
        $firstNames = implode(',', $firstWordNamesArray);
            
            $s1 = $this->conn->prepare("SELECT p2c.product_id FROM  oc_product_to_category p2c 
            LEFT JOIN oc_product p ON (p2c.product_id = p.product_id) 
            LEFT JOIN oc_product_description pd ON (p.product_id = pd.product_id) 
            LEFT JOIN oc_product_to_store p2s ON (p.product_id = p2s.product_id) 
            WHERE p.status = '1' AND p2c.category_id IN(:category_id)
            ORDER BY p.date_added DESC LIMIT :offset , :no_per_page");
            $s1->bindValue(':category_id',  rtrim($categoryIds, ","));
            ...
            LEFT JOIN oc_product_to_store p2s ON (p.product_id = p2s.product_id) 
            WHERE p.status = '1' AND p.date_available <= convert_tz(utc_timestamp(),'-08:00','+0:00') 
            AND p2s.store_id = 0 ORDER BY p.date_added DESC LIMIT :offset , :no_per_page");
            $s1->bindValue(':offset', (int) $offset, PDO::PARAM_INT);
            $s1->bindValue(':no_per_page', $no_per_page, PDO::PARAM_INT);
            $s1->execute();
    ...
    public function globalProducts($page_number){
        ...
        ) p WHERE p.status = 1 ORDER BY p.id DESC LIMIT :offset , :no_per_page");
    ...
    public function brandProductCategory($pageNum, $categoryId, $brandId){
        ...
        $st1 = $this->conn->prepare("SELECT p.*,pd.name FROM oc_product p 
                                    INNER JOIN oc_product_description pd ON p.product_id = pd.product_id
                                    INNER JOIN product_to_brand ptb ON ptb.product_id=p.product_id
                                    INNER JOIN oc_product_to_category PTC ON PTC.product_id=p.product_id
                                    where p.status = 1 AND ptb.brand_id=:b_id AND PTC.category_id=:cid LIMIT :offset , :no_per_page");
        $st1->bindValue(':offset', (int) $offset, PDO::PARAM_INT);
        $st1->bindValue(':no_per_page', $no_per_page, PDO::PARAM_INT);
        $st1->bindValue(':cid', $categoryId);
