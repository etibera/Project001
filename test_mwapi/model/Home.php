...
class Home {
    ...
    public function getStores(){
        global $image;  
        $stmt = $this->conn->prepare("SELECT os.seller_id,os.shop_name,concat('company/',os.image) as image FROM oc_seller os WHERE os.image is not null AND `status` = :status");
    ...
    public function brands(){
            ...
            WHERE pb.status = 1 ORDER BY pb.id desc");
            $stmt->execute();
    ...
    public function most_popular(){
         global $product;
        $data = array();
        $sql = 'SELECT * FROM product_views ORDER BY total_views DESC LIMIT 5';
    ...
    public function recommended($customer_id){
       ...
       $recommended_product = array();
        ...
        $stmt = $this->conn->prepare(" SELECT * FROM (
                                          SELECT p.product_id,pv.p_type,pv.date_viewed,pd.name,p.price,p.image,concat('product.php?product_id=',p.product_id) as href
                                          ...
                                          WHERE pv.customer_id=:customer_id AND pv.p_type=0 and p.status=1 and pv.product_id is not null
                                          UNION ALL
                                          SELECT  p.product_id,pv.p_type,pv.date_viewed,p.product_name as name,p.price,p.img as image,concat('bg_product.php?product_idbg=',p.product_id) as href
                                          FROM product_views pv 
                                          INNER JOIN bg_product p ON p.product_id=pv.product_id
                                          WHERE pv.customer_id=:customer_id AND pv.p_type=2 and p.status=1 and pv.product_id is not null  
                                        ) AS apv
        ...
        // global $product;
        // $data = array();
        // $sql = 'SELECT * FROM product_views WHERE customer_id = :customer_id AND product_id != 0 ORDER BY date_viewed DESC LIMIT 20';
        // $stmt = $this->conn->prepare($sql);
        // $stmt->bindValue(':customer_id', $customer_id);
        // $stmt->execute();
        // foreach($stmt->fetchAll() as $prod){
        ...
        // $stmt = $this->conn->prepare("SELECT cd.category_id,
        ...
        // WHERE hc.customer_id = 0 order by RAND() LIMIT 1");
        // $stmt->execute();
        // $data = $stmt->fetch(PDO::FETCH_ASSOC);
        // return $this->home_product_page($data);
    }
    ...
    public function home_product_page($data = array()){
        global $image;
        $res = array();
        $stmt = $this->conn->prepare("SELECT p2c.product_id, pd.name, p.image, p.price, 'reg' as `type`,
       ...
       FROM  oc_product_to_category p2c  
               ...
               AND p2s.store_id = 0 AND p2c.category_id= :category_id ORDER BY ".$data['sort_order']." LIMIT ".$data['limit']);
        ...
        if($row['cntid'] == 0){
            $stmt = $this->conn->prepare("INSERT INTO product_views 
                                                  SET customer_id = :customer_id, ip = :ip,p_type=:p_type,
                                                      product_id=:product_id,total_views=:total_views,date_viewed=convert_tz(utc_timestamp(),'-08:00','+0:00')");
            $stmt->bindValue(':customer_id', $customer_id);
            $stmt->bindValue(':ip', $_SERVER['REMOTE_ADDR']);
            $stmt->bindValue(':p_type', $p_type);
            $stmt->bindValue(':product_id', $product_id);
            $stmt->bindValue(':total_views', 1);
            $stmt->execute();
        }else{
           $stmt = $this->conn->prepare("UPDATE product_views 
                                                  SET total_views = total_views +:total_views ,date_viewed=convert_tz(utc_timestamp(),'-08:00','+0:00')
