...
class Auth {
    ...
    public function customer($data){
        ...
        return array(
            ...
            'email' => $data['email'],
            'telephone' => $data['telephone'] == 'null' ? '' : "0".substr($data['telephone'], 2),
            'cart' => $data['cart'],
    ...
    public function generate_message($telephone){
        $s = $this->conn->prepare('SELECT * FROM oc_customer where telephone = :telephone LIMIT 1');
        $s->bindValue(':telephone', $telephone);
        $s->execute();
        ...
        if($s->rowCount() > 0){
            ...
            $s1 = $this->conn->prepare("INSERT INTO sms SET 
                ...
                $s1->bindValue(':message', 'Your PESO Verification Code Is '. $data['nexmo_code']);
                $s1->execute();
            return true;
        }else{
            return false;
