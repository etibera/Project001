...
class Cart {
        ...
        if($product_info){
            switch($product_info['type']){
                case 'reg':
                    ...
                    $product_quantity = $this->product_quantity_branch($cart['seller_id'], $product_info['product_id'], $cart['branch_id']);
                    ...
                    $data = array(
                        ...
                        'branchName' =>  $store->get_branch_name($cart['branch_id']),
    ...
    public function add(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            // if($customer_id != 0){
                ...
                $cart_quantity = $this->checkCartQuantity($customer_id, $product_id);
                // echo print_r($p_type . ' type');
                try{
                    if($cart == 0){
                        $sql .= "INSERT oc_cart SET branch_id = :branch_id, deduction_id = :deduction_id, seller_id = :seller_id, price = :price, discount_details = :discount_details, freebies = :freebies,
                        ...
                        $sql_quantity = 'quantity + :quantity';
                        if($selectedProduct->stock == $cart_quantity){
                            $sql_quantity = ':quantity';
                        }
                        $sql .= "UPDATE oc_cart SET branch_id = :branch_id, deduction_id = :deduction_id, quantity = $sql_quantity, seller_id = :seller_id, price = :price, discount_details = :discount_details, freebies = :freebies
    ...
    public function product_quantity_branch($seller_id, $product_id, $branch_id){
        $s = $this->conn->prepare("SELECT * FROM seller_branch_selected_products where product_id = :product_id AND seller_id = :seller_id AND branch_id = :branch_id");
        $s->bindValue(":product_id", $product_id);
        $s->bindValue(":seller_id", $seller_id);
        $s->bindValue(":branch_id", $branch_id);
        $s->execute();
        if($s->rowCount() > 0){
            return (int) $s->fetch()['quantity'];
        }else{
            return 0;
        }
    }
    ...
    public function checkCartQuantity($customer_id, $product_id){
        $s = $this->conn->prepare('SELECT quantity FROM oc_cart WHERE customer_id = :customer_id AND product_id = :product_id LIMIT 1');
        $s->bindValue(':customer_id', (int) trim($customer_id), PDO::PARAM_INT);
        $s->bindValue(':product_id', (int) trim($product_id), PDO::PARAM_INT);
        $s->execute();
        if($s->rowCount() > 0){
            return $s->fetch()['quantity']; 
        }else{
