...
class Home {
    ...
    public function getStoreDetails($store_id){
        global $image;
        $data = array();
        $stmt = $this->conn->prepare("SELECT shop_name, seller_id, concat('company/', image) as image FROM oc_seller WHERE seller_id = :seller_id LIMIT 1");
        ...
        if(isset($_GET['customer_id'])){
            $stmt2 = $this->conn->prepare("SELECT * FROM followers WHERE seller_id = :seller_id AND customer_id = :customer_id");
            $stmt2->bindValue(':seller_id', $store_id);
            $stmt2->bindValue(':customer_id', $_GET['customer_id']);
            $stmt2->execute();
            if($stmt2->rowCount() > 0){
                $followed = true;
            }
        ...
        $data = array(
            ...
            'image' => $image->resize($seller['image'], 75,75),
            'banner' => $img,
            'followed' => $followed,
            'sellerId' => $store_id
        );
    ...
    public function brandProductDetails($brand_id){
            ...
            if($cat['banner_moblie_img'] != null){
                $banner = array($image->resize($cat['banner_moblie_img'], 600, 400));
            }
            
            $followed = false;
            if(isset($_GET['customer_id'])){
                $stmt2 = $this->conn->prepare("SELECT * FROM followers WHERE brand_id = :brand_id AND customer_id = :customer_id");
                $stmt2->bindValue(':brand_id', $brand_id);
                $stmt2->bindValue(':customer_id', $_GET['customer_id']);
                $stmt2->execute();
                if($stmt2->rowCount() > 0){
                    $followed = true;
    ...
    public function latestPromoDetail($lp_id){
        ...
        $data = $this->latestPromoName($lp_id);
        $row['title'] = $data['title'];
        $row['imageName'] = $data['image'];
     ...
     public function lastestPromoProducts($page_number, $id){
        ...
        if($checkSellerCountPromo == 0){
            ...
            $sql .= "SELECT 'reg' as type, spl.deduction_id,sdp.deduction_type,sdp.value,sd.seller_id,
    ...
    public function recommended($customer_id){
       ...
       $recommended_product = array();
        ...
        $stmt = $this->conn->prepare("SELECT * FROM (
                                        ...
                                        GROUP BY pv.product_id,sps.product_id, pv.p_type,pv.date_viewed,pd.name,p.price, p.image ) AS apv 
                                        order by apv.date_viewed DESC limit :offset");
    ...
    public function getSellerLatestPromo($seller_id, $page_number) {
        ...
        $row=array();
        $stmt = $this->conn->prepare("SELECT * FROM (SELECT 'reg' as type, sdp.deduction_type,sdp.value,sd.seller_id,
                                            CASE WHEN (sdp.deduction_type = 0) THEN 'Percentage (%)' ELSE 'PHP' END AS d_type,
									...
									UNION ALL
									SELECT 'reg' as type, sdp.deduction_type,sdp.value,sd.seller_id,
                                            CASE WHEN (sdp.deduction_type = 0) THEN 'Percentage (%)' ELSE 'PHP' END AS d_type,
                                            CASE WHEN (IFNULL(sdp.deduction_type,0)=0) THEN IFNULL(sdp.value,0) / 100  ELSE IFNULL(sdp.value,0) END AS  rate,  os.shop_name, concat('img/company/',os.image) as sellerimage,sdp.product_id,p.price,pd.name,p.image as image,
                                            'Local Product' as typedesc,concat('product.php?product_id=',p.product_id,'&store_id=',sd.seller_id) as href
									FROM seller_deductions sd 
    ...
    public function featuredPromo($status){
        ...
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $value) {
            ...
            $row[] = array(
                ...
                'id' => $value['id'],
                'title' => $value['title'],
                'exclusiveFor' => (int) $value['exclusive_for'],
                'thumb' => $img,
    ...
    public function FeaturedPromoProducts($promo_id,$limit) {
        global $image;
        $row=array();
        $stmt = $this->conn->prepare("SELECT spl.deduction_id,sdp.deduction_type,sdp.value,sd.seller_id,
                                            CASE WHEN (sdp.deduction_type = 0) THEN 'Percentage (%)' ELSE 'PHP' END AS d_type,
                                            CASE WHEN (IFNULL(sdp.deduction_type,0)=0) THEN IFNULL(sdp.value,0) / 100  ELSE IFNULL(sdp.value,0) END AS  rate,
                                            os.shop_name, concat('img/company/',os.image) as sellerimage,sdp.product_id,p.price,pd.name,p.image as image,
    ...
    public function brandFollow($customer_id, $brand_id){
        $stmt1 = $this->conn->prepare("SELECT * FROM followers WHERE customer_id=:customer_id AND brand_id=:brand_id");
        $stmt1->bindValue(':brand_id', $brand_id);
        $stmt1->bindValue(':customer_id', $customer_id);
        $stmt1->execute();
        if($stmt1->rowCount() > 0){
            $stmt2 = $this->conn->prepare("DELETE FROM followers WHERE customer_id=:customer_id AND brand_id=:brand_id");
            $stmt2->bindValue(':brand_id', $brand_id);
            ...
            $stmt3 = $this->conn->prepare("INSERT INTO followers SET customer_id=:customer_id, brand_id=:brand_id");
            $stmt3->bindValue(':brand_id', $brand_id);
