...
class Checkout {
    ...
    public function checkoutDetails($cart_ids, $customer_id, $total_price){
        global $address;
        $cart_ids = implode(",", json_decode(htmlspecialchars_decode($cart_ids), true));
        $data = array();
        // $data['store'] = $this->get_cart_local_perstore($customer_id, $cart_ids);
        $large_products = $this->large_products_query($customer_id,$cart_ids);
        $small_products = $this->small_products_query($customer_id,$cart_ids);
        $data['products'] = $this->filterProducts($large_products, $small_products, $cart_ids, $customer_id);
        $data['paymentMethod'] = $this->paymentMethod($cart_ids, $customer_id, $total_price);
    ...
    public function large_products_query($customer_id, $cart_ids){
        $select_stmt = $this->conn->prepare("SELECT pdc.delivery_charge_id,oc.*,oc.quantity as qtyoc,
                                                oc.price * oc.quantity AS total_price,sb.b_name as shop_name,
                                                sb.branch_logo  as image,p.image as img,p.product_id,pd.name,p.model,p.price as oprice,dc.amount  as FlatrateSamePRV,dc.provincial_amount  as  FlatrateDiffPRV
                                            FROM oc_cart oc
                                            INNER JOIN oc_seller os 
                                                ON oc.seller_id = os.seller_id  
                                            INNER JOIN seller_branch sb 
                                                ON sb.id=oc.branch_id AND sb.seller_id=oc.seller_id
                                            ...
                                            INNER JOIN oc_delivery_charge dc 
                                                ON pdc.delivery_charge_id=dc.id 
                                            WHERE oc.customer_id=:customer_id  and dc.id!=5 and oc.p_type=:p_type and oc.cart_id  IN (".$cart_ids.")");
        $select_stmt->bindValue(':customer_id', $customer_id);
        $select_stmt->bindValue(':p_type',0);
        $select_stmt->execute();        
        return $select_stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    public function small_products_query($customer_id,$cart_ids){
        $select_stmt = $this->conn->prepare("SELECT sb.b_name as shop_name,oc.seller_id,
        sb.branch_logo  as image,oc.branch_id
            FROM oc_cart oc
            INNER JOIN oc_seller os 
                ON oc.seller_id = os.seller_id  
            INNER JOIN seller_branch sb 
            ...
            INNER JOIN oc_product_delivery_charge pdc 
                ON oc.product_id=pdc.product_id 
            WHERE oc.customer_id=:customer_id and oc.p_type=:p_type and pdc.delivery_charge_id=5 and oc.cart_id  IN (".$cart_ids.")
            group by oc.seller_id,sb.branch_logo,sb.id,sb.b_name");
        $select_stmt->bindValue(':customer_id', $customer_id);
        $select_stmt->bindValue(':p_type',0);
        $select_stmt->execute();        
        return $select_stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    public function get_cart_perstore($large_products, $small_products, $customer_id,$cart_idstr, $address_id){
        foreach ($large_products as $row) {
            $insurance_fee=0;
            $total_price=$row['total_price'];
            if($total_price>500){  
                $insurance_fee=$total_price*0.01;              
            }
            $stores[] = array(
                'sellerId' => $row['seller_id'],
                'creditCard' => round(($total_price / .972) - $total_price, 2),
                'maxxPayment' => round($total_price * 0.015, 2),
                'flatRate' => $this->flatRateLarge($row, $address_id),
        ...
        foreach ($small_products as $row) {
            $insurance_fee=0;
            $total_perstore=$this->StoreCartTotalSmall($customer_id,$cart_idstr,$row['seller_id'],$row['branch_id']);
            if($total_perstore>500){  
                $insurance_fee=$total_perstore*0.01;              
            }
            $stores[] = array(
                'sellerId' => $row['seller_id'],
                'creditCard' => round(($total_perstore / .972) - $total_perstore, 2),
                'maxxPayment' => round($total_perstore * 0.015, 2),
                'flatRate' => $this->flatRateSmall($customer_id,$cart_idstr,$row['seller_id'],$row['branch_id'],$address_id),
                'subTotal' => round($total_perstore, 2),
                'branchId' => (int)$row['branch_id'],
                'insuranceFee' => $insurance_fee,
                'type' => 'small'
            );
    ...
    public function filterProducts($large_products, $small_products, $cart_idstr, $customer_id){
        global $image;
        foreach ($large_products as $row) {
            $stores['large'][] = array(
                ...
                'branchId' => (int)$row['branch_id'],
                'price' =>$row['price'],
                'logo' => $image->resize($row['image'], 20,20),
                'img' => $image->resize($row['img'], 200,200),
        ...
        foreach ($small_products as $row) {
            ...
            $stores['small'][] = array(
                ...
                'sellerId' => (int)$row['seller_id'],
                'branchId' => (int)$row['branch_id'],
                'subtotal' => (float)$total_perstore,
                'logo' => $image->resize($row['image'], 20,20),
                'products' => $this->getProductSmallPerStore($customer_id,$cart_idstr,$row['seller_id'],$row['branch_id']),
            );
        }
        return $stores;
    }
    public function getProductSmallPerStore($customer_id,$cart_idstr,$seller_id,$branch_id){
        global $image;
        $data = array();
        $select_stmt = $this->conn->prepare("SELECT pdc.delivery_charge_id,oc.*,oc.quantity as qtyoc,
                                                oc.price * oc.quantity AS total_price,p.image as img,p.product_id,pd.name,p.model,p.price as oprice
                                            FROM oc_cart oc
                                            INNER JOIN oc_product p 
                                                ON oc.product_id = p.product_id 
                                            INNER JOIN oc_product_description pd 
                                            ...
                                            INNER JOIN oc_product_delivery_charge pdc 
                                                ON oc.product_id=pdc.product_id 
                                            WHERE oc.customer_id=:customer_id  AND p.status = '1' AND oc.p_type=:p_type AND oc.seller_id=:seller_id AND oc.branch_id=:branch_id AND pdc.delivery_charge_id=5 AND oc.cart_id  IN (".$cart_idstr.")");
        $select_stmt->bindValue(':customer_id', $customer_id);
        $select_stmt->bindValue(':p_type',0);
        $select_stmt->bindValue(':seller_id',$seller_id);
        $select_stmt->bindValue(':branch_id',$branch_id);
        $select_stmt->execute();
    ...
    public function flatRateLarge($row,$address_id){
        $GetshipAddress=$this->GetshipAddress($address_id);
        $pickupAddress=$this->GetPickupAddress($row['branch_id']);           
        $flat_rate=0;
        $FlatrateSamePRV=0;
        $FlatrateDiffPRV=0;
        

        if($row['delivery_charge_id']==5){
            //small                
            if($row['qtyoc']<5){
                //small
                $getsmallRate=$this->getDRRate(5);
                $FlatrateSamePRV=$getsmallRate['amount'];
                $FlatrateDiffPRV=$getsmallRate['provincial_amount'];
            }else if($row['qtyoc']<9){
                //meduim
                $getmeduimRate=$this->getDRRate(1);
                $FlatrateSamePRV=$getmeduimRate['amount'];
                $FlatrateDiffPRV=$getmeduimRate['provincial_amount'];
            }else if($row['qtyoc']<17){
                //large
                $getlageRate=$this->getDRRate(2);
                $FlatrateSamePRV=$getlageRate['amount'];
                $FlatrateDiffPRV=$getlageRate['provincial_amount'];
            }else{
                //box 
                $getboxRate=$this->getDRRate(3);
                $FlatrateSamePRV=$getboxRate['amount'];
                $FlatrateDiffPRV=$getboxRate['provincial_amount'];
            }

        }else if($row['delivery_charge_id']==1){
            //for meduim
            $FlatrateSamePRV=$row['FlatrateSamePRV']*$row['qtyoc'];
            $FlatrateDiffPRV=$row['FlatrateDiffPRV']*$row['qtyoc'];
        }else if($row['delivery_charge_id']==2){
            //for large
            $FlatrateSamePRV=$row['FlatrateSamePRV']*$row['qtyoc'];
            $FlatrateDiffPRV=$row['FlatrateDiffPRV']*$row['qtyoc'];
        }else{
        ...
        if($GetshipAddress['region']==$pickupAddress['region']){
            //"Not provincial rate";
            $flat_rate =  $FlatrateSamePRV;
        }else{
            //"provincial rate";
            $flat_rate = $FlatrateDiffPRV;
        }  
        return $flat_rate;   
    }
    public function flatRateSmall($customer_id,$cart_idstr,$seller_id,$branch_id,$custDeffAdd){
        $subtotal = array();
        $select_stmt = $this->conn->prepare("SELECT sum(oc.quantity) as totalqty
                                            FROM oc_cart oc
                                            INNER JOIN oc_product_delivery_charge pdc 
                                                ON oc.product_id=pdc.product_id 
                                            WHERE oc.customer_id=:customer_id and oc.p_type=:p_type AND oc.seller_id=:seller_id AND oc.branch_id=:branch_id and pdc.delivery_charge_id=5 AND oc.cart_id  IN (".$cart_idstr.") ");
        $select_stmt->bindValue(':customer_id', $customer_id);
        $select_stmt->bindValue(':p_type',0);
        $select_stmt->bindValue(':seller_id',$seller_id);
        $select_stmt->bindValue(':branch_id',$branch_id);
        ...
        $GetshipAddress=$this->GetshipAddress($custDeffAdd);
        ...
        if($data['totalqty']<5){
            ...
            $FlatrateSamePRV=$getsmallRate['amount'];
            $FlatrateDiffPRV=$getsmallRate['provincial_amount'];
        }else if($data['totalqty']<9){
            //meduim
            $getmeduimRate=$this->getDRRate(1);
            $FlatrateSamePRV=$getmeduimRate['amount'];
            $FlatrateDiffPRV=$getmeduimRate['provincial_amount'];
        }else if($data['totalqty']<17){
            //large
            $getlageRate=$this->getDRRate(2);
            $FlatrateSamePRV=$getlageRate['amount'];
            $FlatrateDiffPRV=$getlageRate['provincial_amount'];
    ...
    public function getDRRate($delivery_charge_id){
        $data = array();
        $stm = $this->conn->prepare("SELECT * FROM oc_delivery_charge
                                    WHERE id=:delivery_charge_id");
        $stm->bindValue(':delivery_charge_id', $delivery_charge_id);
        $stm->execute();
        $data = $stm->fetch(PDO::FETCH_ASSOC);         
        return $data;
    }
    
    public function StoreCartTotalSmall($customer_id,$cart_idstr,$seller_id,$branch_id){
        $subtotal = array();
    ...
    public function checkoutGetLargeProducts($customer_id,$cart_ids, $payment_method, $address_id, $shippingCode, $branch_ids, $seller_ids, $order_id, $skip_shipping_insurance){
        $stores = array();
        $orderNo = 0;
        $shippingValues = ['special_del', 'pickup.pickup'];
        // $shippingValue = array_sum(array_column($shipMethoData['stores'], 'flatRate'));
        
        foreach ($this->large_products_query($customer_id, $cart_ids) as $row) {
            $orderNo++;
            $insurance_fee=0;
            $total_price=$row['total_price'];
            if($total_price>500){  
            ...
            $largeFlatRate = $this->flatRateLarge($row, $address_id);
            $showSSR = $this->show_ssr($branch_ids, $address_id);
            $shipping = $this->show_options($showSSR, $largeFlatRate, $seller_ids);
            $shippingIndex = array_search($shippingCode, array_column($shipping, 'code'));
            $selectedShipping = $shipping[$shippingIndex];
            $total_per_store_final_total = 0;
            $stores['store_orders'][] = array(
                'seller_id' => $row['seller_id'],
                'branch_id' => $row['branch_id'],
                'orderNo' => $order_id."-".$orderNo,
                'orderId' => $order_id
            );
            $stores['shipping_per_store'][] = array(
                'orderNo' => $order_id."-".$orderNo,
                'orderId' => $order_id,
                'seller_id' => $row['seller_id'],
                'branch_id' => $row['branch_id'],
                'value' => $selectedShipping['total'],
                'title' => $selectedShipping['name'],
                'orderId' => $order_id
            );
            $stores['total_per_store'][] = array(
                'orderNo' => $order_id."-".$orderNo,
                'orderId' => $order_id,
                'sellerId' => $row['seller_id'],
            ...
            $total_per_store_final_total += (float)$row['total_price'];
            if($shippingCode == 'shipping_fee'){
                $total_per_store_final_total += (float)$selectedShipping['total'];
                $stores['total_per_store'][] = array(
                    'orderNo' => $order_id."-".$orderNo,
                    'orderId' => $order_id,
                    'sellerId' => $row['seller_id'],
            ...
            if($insurance_fee > 0 && !in_array($shippingCode, $shippingValues)){
                if($skip_shipping_insurance == 0){
                    $total_per_store_final_total += $insurance_fee;
                    $stores['total_per_store'][] = array(
                        'orderNo' => $order_id."-".$orderNo,
                        'orderId' => $order_id,
                        'sellerId' => $row['seller_id'],
            ...
            $otherCharge = 0;
            $credit_card = round(($row['total_price']/.972)-$row['total_price'],2);
            $maxx_payment = round($row['total_price']*0.015,2);
            if($payment_method['value'] == 'credit_card'){
                $otherCharge = $credit_card;
            }
            if($payment_method['value'] == 'maxx_payment'){
                $otherCharge = $maxx_payment;
            }
            if($otherCharge > 0){
                $total_per_store_final_total += $otherCharge;
                $stores['total_per_store'][] = array(
                    'orderNo' => $order_id."-".$orderNo,
                    'orderId' => $order_id,
                    'sellerId' => $row['seller_id'],
            ...
            $stores['total_per_store'][] = array(
                ...
                'branchId' => $row['branch_id'],
                'title' => 'Total',
                'value' => $total_per_store_final_total,
                'code' => 'total',
                'sort' => 8
            );
        ...
        foreach ($this->small_products_query($customer_id, $cart_ids) as $row) {
            $orderNo++;
            $insurance_fee=0;
            $smallFlatRate = $this->flatRateSmall($customer_id,$cart_ids,$row['seller_id'],$row['branch_id'],$address_id);
            $total_price=$this->StoreCartTotalSmall($customer_id,$cart_ids,$row['seller_id'],$row['branch_id']);
            $showSSR = $this->show_ssr($branch_ids, $address_id);
            $shipping = $this->show_options($showSSR, $smallFlatRate, $seller_ids);
            $shippingIndex = array_search($shippingCode, array_column($shipping, 'code'));
            $selectedShipping = $shipping[$shippingIndex];
            $total_per_store_final_total_s = 0;
            if($total_price>500){  
                $insurance_fee=$total_price*0.01;              
            }
            $stores['store_orders'][] = array(
            ...
            $total_per_store_final_total_s += (float)$total_price;
            if($shippingCode == 'shipping_fee'){
                $total_per_store_final_total_s += (float)$selectedShipping['total'];
                $stores['total_per_store'][] = array(
                    'orderNo' => $order_id."-".$orderNo,
                    'orderId' => $order_id,
                    'sellerId' => $row['seller_id'],
            ...
            if($insurance_fee > 0 && !in_array($shippingCode, $shippingValues)){
                if($skip_shipping_insurance == 0){
                    $total_per_store_final_total_s += $insurance_fee;
                    $stores['total_per_store'][] = array(
                        'orderNo' => $order_id."-".$orderNo,
                        'orderId' => $order_id,
                        'sellerId' => $row['seller_id'],
            ...
            $otherCharge = 0;
            $credit_card = round(($total_price/.972)-$total_price,2);
            $maxx_payment = round($total_price*0.015,2);
            if($payment_method['value'] == 'credit_card'){
                $otherCharge = $credit_card;
            }
            if($payment_method['value'] == 'maxx_payment'){
            ...
            if($otherCharge > 0){
                $total_per_store_final_total_s += $otherCharge;
                $stores['total_per_store'][] = array(
                    'orderNo' => $order_id."-".$orderNo,
                    'orderId' => $order_id,
                    'sellerId' => $row['seller_id'],
            ...
            $stores['total_per_store'][] = array(
                ...
                'branchId' => $row['branch_id'],
                'title' => 'Total',
                'value' => $total_per_store_final_total_s,
                'code' => 'total',
                'sort' => 8
            );
            foreach($this->getProductSmallPerStore($customer_id,$cart_ids,$row['seller_id'],$row['branch_id']) as $product){
                $stores['products'][] = array(
                    'orderNo' => $order_id."-".$orderNo,
                    'orderId' => $order_id,
    ...
    public function get_cart_local_perstoreSmall($customer_id,$cart_ids,$address_id){
        $stores = array();
        global $image;
        $stores = array();
        ...
        $select_stmt = $this->conn->prepare("SELECT sb.b_name as shop_name,oc.seller_id,
                                            ...
                                            INNER JOIN oc_product_delivery_charge pdc 
                                                ON oc.product_id=pdc.product_id 
                                            WHERE oc.customer_id=:customer_id and oc.p_type=:p_type and pdc.delivery_charge_id=5 and oc.cart_id  IN (".$cart_idstr.")
                                            group by oc.seller_id,sb.branch_logo,sb.id,sb.b_name");
        $select_stmt->bindValue(':customer_id', $customer_id);
        $select_stmt->bindValue(':p_type',0);
        $select_stmt->execute();        
        ...
        foreach ($storeData as $row) {
            ...
            $stores[] = array(
                ...
                'seller_id' => $row['seller_id'],
                'branch_id' => $row['branch_id'],
                'credit_card' =>round(($total_perstore/.972)-$total_perstore,2),
                'maxx_payment' =>round($total_perstore*0.015,2),
                'delstats' => $this->deliveryStatus($address_id),
                'pickupstats' => $this->pickupStatus($row['branch_id']),
                'flatrate' => $this->flatrateSmall($customer_id,$cart_idstr,$row['seller_id'],$row['branch_id'],$custDeffAdd),
                'subtotal' =>$total_perstore,
                'insurance_fee' =>$insurance_fee,
                'branch_logo' => $row['image'],
                'details' => $this->getProductSmallPerStore($customer_id,$cart_idstr,$row['seller_id'],$row['branch_id']),
            );
        }
        return $stores; 
    }
    ...
    public function get_cart_product_installment($customer_id,$cart_ids){
        ...
        $select_stmt = $this->conn->prepare(" SELECT count(oc.cart_id) as count from oc_cart oc
                                            INNER JOIN seller_branch_selected_products sps 
                                                ON sps.product_id=oc.product_id AND sps.seller_id=oc.seller_id AND sps.branch_id=oc.branch_id
    ...
    public function paymentMethod($cart_ids, $customer_id, $total_price){
        $codAmountValidate = $this->validateAmount($total_price);
        $installment = $this->get_cart_product_installment($customer_id, $cart_ids);
        $codSellerValidate = $this->get_cart_product_cod($customer_id,$cart_ids);
        $disabledCod = true;
		...
		if($this->isLandbank($customer_id)){
		    $pm[] = array(
                'name' => 'LANDBANKPay',
                'value' => 'land_bank_pay',
                'disable' => false
		...
		if($this->isFourgives($customer_id)){
            $pm[] = array(
...
// 		);
    ...
    public function addOrder($token){
        ...
        $cart_ids = implode(",", json_decode(htmlspecialchars_decode($_POST['cart_ids']), true));
        // $totalsPerStore = json_decode(htmlspecialchars_decode($_POST['totals_per_store']), true);
        $branch_ids = json_decode(htmlspecialchars_decode($_POST['branch_ids']), true);
        $seller_ids = json_decode(htmlspecialchars_decode($_POST['seller_ids']), true);
        
        
        $skip_shipping_insurance = 0;
        if(isset($_POST['has_shipping_insurance'])){
        if($_POST['has_shipping_insurance'] == 'true'){
                $skip_shipping_insurance = 0;
            }else{
                $skip_shipping_insurance = 1;
                }
            }
        //  return   $this->checkoutGetLargeProducts($customer_id, $cart_ids, $payment_method, $_POST['shipping_address_id'], $_POST['shippingCode'], $branch_ids, $seller_ids, 1999, $skip_shipping_insurance);
        try{
            ...
            skip_shipping_insurance = :skip_shipping_insurance
            ");
            ...
            $stm->bindValue(':skip_shipping_insurance', $skip_shipping_insurance);
            $stm->execute();
            ...
            // insert
            $stores = $this->checkoutGetLargeProducts($customer_id, $cart_ids, $payment_method, $_POST['shipping_address_id'], $_POST['shippingCode'], $branch_ids, $seller_ids, $lastId, $skip_shipping_insurance);
            $this->SaveStoreOrders($stores['store_orders']);
            $this->insertOrderProduct($stores['products']);
            $this->insertOrderTotal($totals, $lastId);
            $this->SaveTotalsPerStore($stores['total_per_store']);
            $this->SaveShippingPerStore($stores['shipping_per_store']);
            ...
            if($order_status_id !== 0){
                if($this->bgProductCount($lastId, 2) > 0){
                  ...
                  if($res == '200'){
                    $this->deleteFromCart($cart_ids, $customer_id);
    ...
    public function SaveShippingPerStore($shipping_per_store){
        foreach($shipping_per_store as $sps){
            if($sps['title']!="Global Products"){
                $add_sps= $this->conn->prepare("INSERT INTO order_shipping_per_sotre  SET order_id = :order_id, title = :title, value = :value, seller_id = :seller_id,branch_id = :branch_id, order_number = :order_number");
                $add_sps->bindValue(':order_id', $sps['orderId']);
                $add_sps->bindValue(':title', $sps['title']);
                $add_sps->bindValue(':value', $sps['value']);                                
                $add_sps->bindValue(':seller_id', $sps['seller_id']);
                $add_sps->bindValue(':branch_id', $sps['branch_id']);
                $add_sps->bindValue(':order_number', $sps['orderNo']);
                $add_sps->execute(); 
            }             
        }
    }
    public function SaveStoreOrders($store_orders){
        foreach($store_orders as $so){
            $add_SO = $this->conn->prepare("INSERT INTO store_orders SET order_id = :order_id, seller_id = :seller_id, branch_id = :branch_id, order_number = :order_number");
            $add_SO->bindValue(':order_id', $so['orderId']);
            $add_SO->bindValue(':seller_id', $so['seller_id']);
            $add_SO->bindValue(':branch_id', $so['branch_id']);
            $add_SO->bindValue(':order_number', $so['orderNo']);
            $add_SO->execute();
            $updatesps = $this->conn->prepare("INSERT INTO order_status_per_store 
                                                SET order_id =:order_id,seller_id=:seller_id,
                                                order_status_id = '17',branch_id = :branch_id, order_number = :order_number");
            $updatesps->bindValue(':order_id', $so['orderId']);
            $updatesps->bindValue(':seller_id', $so['seller_id']);
            $updatesps->bindValue(':branch_id', $so['branch_id']);
            $updatesps->bindValue(':order_number', $so['orderNo']);
            $updatesps->execute();
        }
    }
    public function SaveTotalsPerStore($total_per_store){
        foreach($total_per_store as $tps){
            $sub_total = $this->conn->prepare("INSERT INTO order_total_per_store SET order_id = :order_id,seller_id=:seller_id, code = :code, title = :title, value = :value, sort_order = :sort_order,branch_id = :branch_id, order_number = :order_number");
            $sub_total->bindValue(':order_id', $tps['orderId']);
            ...
            $sub_total->bindValue(':branch_id', $tps['branchId']);                 
            $sub_total->bindValue(':order_number', $tps['orderNo']);                 
            $sub_total->bindValue(':sort_order', $tps['sort']);
            $sub_total->execute(); 
        }
    }
    ...
    public function insertOrderProduct($products){
        global $cart;
        global $banggood;
        foreach($products as $row){
            if($row['p_type'] == 0){
                $add_product = $this->conn->prepare("INSERT INTO oc_order_product SET order_id = :order_id, product_id = :product_id, name = :name, model = :model, quantity = :quantity, price = :price, total = :total, tax = 0, reward = 0,p_type=:p_type,shipping_fee=:shipping_fee,shipping_name=:shipping_name,shipping_code=:shipping_code,poa_name=:poa_name,poa_ids=:poa_ids,warehouse_code=:warehouse_code,seller_id=:seller_id,discount_details=:discount_details,freebies=:freebies,branch_id = :branch_id, order_number = :order_number");
                $add_product->bindValue(':order_id', $row['orderId']);
                $add_product->bindValue(':product_id', $row['product_id']);
                $add_product->bindValue(':name', $row['name']);
                $add_product->bindValue(':model', $row['model']);
                $add_product->bindValue(':quantity', $row['quantity']);
                $add_product->bindValue(':price', $row['price']);
                $add_product->bindValue(':total', $row['total_price']);
                $add_product->bindValue(':p_type', $row['p_type']);
                $add_product->bindValue(':shipping_fee', floatval($row['shipping_fee']));
                $add_product->bindValue(':shipping_name', $row['shipping_name']);
                $add_product->bindValue(':shipping_code', $row['shipping_code']);
                $add_product->bindValue(':poa_name', $row['poa_name']);
                $add_product->bindValue(':poa_ids', $row['poa_ids']);
                $add_product->bindValue(':warehouse_code', $row['warehouse_code']);
                $add_product->bindValue(':seller_id', $row['seller_id']);
                $add_product->bindValue(':discount_details', $row['discount_details']);
                $add_product->bindValue(':freebies', $row['freebies']);
                $add_product->bindValue(':branch_id', $row['branch_id']);                 
                $add_product->bindValue(':order_number', $row['orderNo']); 
                $add_product->execute();
            }else if($product['p_type'] == 2){
                $ship = $banggood->getShipping($row['product_id'], $cart->get_bg_warehouse($row['product_id']), (int)$product['quantity']);
                $poa_list = $this->getBGPoaList($row['cart_id'], $row['product_id']);
                ...
                $order_product->bindValue(':product_id', $row['product_id'], PDO::PARAM_INT);
                $order_product->bindValue(':name', $row['name']);
                $order_product->bindValue(':model', $row['model']);
                $order_product->bindValue(':quantity', $row['quantity']);
                $order_product->bindValue(':price', $row['price']);
                $order_product->bindValue(':total', $row['total']);
                $order_product->bindValue(':p_type', $row['p_type']);
    ...
    public function deleteFromCart($cart_ids, $customer_id){
        $delete = $this->conn->prepare("DELETE from oc_cart where customer_id = :customer_id AND cart_id IN (".$cart_ids.")");
        $delete->bindValue(':customer_id', $customer_id, PDO::PARAM_INT);
        // $delete->bindValue(':cart_ids', $cart_ids);
        $delete->execute();
    }
    ...
    public function getSelectedCartProduct($customer_id, $cart_ids){
        global $cart;
        global $product;
        global $banggood;
        $data = array();
        $st = $this->conn->prepare('SELECT * FROM oc_cart c WHERE c.customer_id = :customer_id AND c.cart_id IN ('.$cart_ids.')');
        // $st->bindValue(':cart_ids', $cart_ids);
    ...
    public function pickupStatus($branch_id){
        $data = array();
        $stm = $this->conn->prepare("SELECT addt.pickup FROM seller_branch sb
    ...
    public function shippingMethodList($address_id, $cart_ids, $customer_id, $branch_ids, $seller_ids, $total_price){
        ...
        $large_products = $this->large_products_query($customer_id,$productIds);
        $small_products = $this->small_products_query($customer_id,$productIds);
        $shipMethoData['stores'] = $this->get_cart_perstore($large_products, $small_products, $customer_id, $productIds, $address_id);

        $shippingValue = array_sum(array_column($shipMethoData['stores'], 'flatRate'));
        $shipMethoData['option'] = $this->show_options($showSSR, $shippingValue, $seller_ids);
        // $shipMethoData['sellerFlatRate'] = array();

        $shipMethoData['cod'] = $this->get_cod($total_price, $customer_id, $productIds, $showSSR);
        

        return $shipMethoData;
    }
    public function show_options($showSSR, $shippingValue, $seller_ids ){
        $shipMethoData = array();
        if($showSSR){
            $shipMethoData[] = array(
                'name' => 'Standard Shipping Rate',
        ...
        $shipMethoData[] = array(
            ...
            'total' => 0,
            'value' => 'special_del',
            'note' => 'Customer handles shipping / Approximately Same day or next day delivery With higher rate',
            'disable' => false
        );
        $showPickUp = false;
    ...
    public function show_ssr($branch_ids, $address_id){
        $delstats = $this->deliveryStatus($address_id);	
        $showSSR = false;
        if($address_id){
    ...
    public function get_cod($total_price, $customer_id, $productIds, $showSSR){
        $disabled = false;
        $codMsg = '';
        $codAmountValidate = $this->validateAmount($total_price);
    ...
    public function isLandbank($customer_id){
        $data = array();
        $stm = $this->conn->prepare("SELECT landbankacc FROM oc_customer where customer_id = :customer_id");
        $stm->bindValue(':customer_id', $customer_id);
        $stm->execute();
        return $stm->fetch()['landbankacc'];
    }
    public function isFourgives($customer_id){
        $data = array();
        $stm = $this->conn->prepare("SELECT fgivesacc FROM oc_customer where customer_id = :customer_id");
        $stm->bindValue(':customer_id', $customer_id);
        $stm->execute();
        return $stm->fetch()['fgivesacc'];
    }
        public  function getTotalOrder($order_id){
        $t = $this->conn->prepare("SELECT value as total from oc_order_total where order_id=:order_id AND title='Total' ");
       ...
       public function getFourgivesURL($lastId, $customer_id, $live){
            $p2mApi=new p2mApi;
            ...
            $TotalOrderAmt =round($this->getTotalOrder($lastId), 2);  
            $order_id = $lastId;                  
            ...
            $saveP2mRES = $this->conn->prepare("INSERT INTO p2m_response SET order_id = :order_id, merc_token = :merc_token, qrph = :qrph");
            $saveP2mRES->bindValue(':order_id', $lastId);
            $saveP2mRES->bindValue(':merc_token', $attrs["merc_token"]);
            $saveP2mRES->bindValue(':qrph', $attrs["qrph"]);
            $saveP2mRES->execute();
            $token=base64_encode($lastId);
            //$live=1;
            return "generateQR.php?live=".$live."&order_id=".$lastId."&token=".$token."&userId=".$customer_id."&t=".uniqid(); 
    }
}
