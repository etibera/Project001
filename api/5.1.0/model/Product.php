...
class Product {
    ...
    public function localProduct($product_id, $storeId){
       ...
       if($select_product->rowCount()){
           $this->generateReview($product_id);
            if($data['image']){
                $img = $image->img_path($data['image']);
            }else{
                $img = '';
            }
            ...
            if($storeId){
                ...
                if(count($storeList[0]['deductionData']) > 0){
                    
                    $oldPrice = $newPrice;
                    
                    $deductionData = $storeList[0]['deductionData'][0];
                    $deduction_type = $deductionData['deduction_type'];
                    $rate = $deductionData['rate'];
                    //if($deductionData['commingsoon'] == 0){
                        if($deduction_type !== null){
                        if($deduction_type == 0){
                            $priceDeduct = ((float)$rate * 100) . "% OFF";
                        }else{
                            $priceDeduct = "₱".(int)$rate. " OFF";
                        }
                        $oldPrice = $newPrice;
                        if($deduction_type == 0 && $rate != 0){
                          $newPrice = $oldPrice - ($newPrice * (float)$rate);
                        }else if($deduction_type == 1 && $rate != 0){
                          $newPrice = $oldPrice - ((float)$rate);
                        }
                    //}
                    }
                }
    ...
    public function get_store_list($product_id, $storeId, $price){   
        $row = array();
            global $image;
            if($storeId !== 'null'){
                $stmt = $this->conn->prepare("SELECT os.seller_id,sb.b_name as shop_name,sbsp.branch_id,
                sbsp.brand_id,opd.name,sbsp.quantity as qty,
                sb.branch_logo as image  
            ...
            WHERE sbsp.quantity!=0 AND sbsp.product_id=:product_id AND os.seller_id=:store_id
       ...
       foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $value) {
           $freebiesdata=$this->Getfreebies($product_id,$value['seller_id'],$value['branch_id']);
            $freebies="";
    
            if($freebiesdata){
                $freebies=$freebiesdata['description'];
            }
            $row[] = array(
               'image' => $value['image'],
   ...
   public function Getfreebies($product_id,$seller_id,$branch_id){
        $row=array();
        $stmt = $this->conn->prepare("SELECT *  FROM seller_product_freebies  where product_id =:product_id AND seller_id=:seller_id AND branch_id=:branch_id ");
        $stmt->bindValue(':product_id',$product_id);
    ...
    public function get_deductions_per_seller($seller_id,$product_id, $price){
      global $image;
         $row = array();
        $stmt = $this->conn->prepare("SELECT sd.id,IFNULL(sdp.deduction_type,0) as deduction_type,sd.description,
                                        sdp.value, sd.date_from,sd.date_to,
                                        CASE WHEN (IFNULL(sdp.deduction_type,0)=0) THEN IFNULL(sdp.value,0) / 100  ELSE IFNULL(sdp.value,0) END AS  rate ,
        ...
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $value) {
                ...
                'pricePromoText' => $priceDeduct,
                'exclusiveFor' => $this->getExclusiveFor($value['id']),
                'commingsoon' => 0,
                'type'=> 'radio',
                'checked' => false
               );
        } 
         $stmtFUTURE = $this->conn->prepare("SELECT sd.id,IFNULL(sdp.deduction_type,0) as deduction_type,sd.description,
                                        sdp.value, sd.date_from,sd.date_to,
                                        CASE WHEN (IFNULL(sdp.deduction_type,0)=0) THEN IFNULL(sdp.value,0) / 100  ELSE IFNULL(sdp.value,0) END AS  rate ,
                                        DATE_FORMAT(sd.date_from, '%M %d %Y') as date_f,DATE_FORMAT(sd.date_to, '%M %d %Y') as date_t
                                    FROM seller_deductions sd 
        ...
        foreach ($stmtFUTURE->fetchAll(PDO::FETCH_ASSOC) as $value1) {
            ...
            if($current_date >= $start_date){
                ...
                $datePromoText = "<div class='red-color'>Discounted from <b>".$value1['date_f']. "</b><br> to <b>". $value1['date_t']. "</b></div>";
             ...
             $row[] = array(
                ...
                'exclusiveFor' => $this->getExclusiveFor($value1['id']),
                'pricePromoText' => '',
                'commingsoon' => 1,
                'type'=> 'radio',
               'checked' => false
   ...
   public function getExclusiveFor($deduction_id){
        $st = $this->conn->prepare("SELECT lp.exclusive_for  FROM `latest_promo` lp 
        INNER JOIN latest_promo_seller_list lpsl ON  lpsl.latest_promo_id=lp.id
        INNER JOIN lp_seller_promo_list lsp ON lsp.seller_list_id=lpsl.id
        INNER JOIN seller_deductions_product sdp ON sdp.deduction_id=lsp.deduction_id
        where sdp.deduction_id=:deduction_id and lp.status=1 LIMIT 1");
        $st->bindValue(':deduction_id', $deduction_id);
        $st->execute();
        switch($st->fetch()['exclusive_for']){
            case '0':
                return null;
            break;
            case '1':
    ...
    public function get_product_images($product_id, $main_image){
        ...
        foreach($st->fetchAll() as $images){
            ...
            $data[] = $image->img_path($images['image']);
        }
    ...
    public function findAllProduct($searchval,$data){  
        ...
        $sql="SELECT s.product_id,s.price,s.name,s.image,'reg' as type,s.typedesc,s.href 
            FROM ( SELECT p.date_modified as date_m,p.product_id,p.price,pd.name,p.image as image,'Local Product' as typedesc,'0' as type,concat('product.php?product_id=',p.product_id) as href
                FROM oc_product p 
                INNER JOIN oc_product_description pd ON (p.product_id = pd.product_id) 
                WHERE (pd.name like :searchval OR p.model like :searchval OR pd.description like :searchval) AND p.status =:s                             ";
        if(isset($data['price_range'])){
            if(count($data['price_range']) != 0){
                $sql .= " AND p.price ";
                if($data['price_range'][1] == 'Above'){
  ...
  global $image;
    ...
    public function getSellerLatestPromoWithCategory($pageNum, $sellerid,$category_id) {
        global $image;
        $no_per_page = 12;
        $offset = ($pageNum-1) * $no_per_page;
        $row=array();
        $stmt = $this->conn->prepare("SELECT sdp.deduction_type,sdp.value,sd.seller_id,
                                            CASE WHEN (sdp.deduction_type = 0) THEN 'Percentage (%)' ELSE 'PHP' END AS d_type,
                                            CASE WHEN (IFNULL(sdp.deduction_type,0)=0) THEN IFNULL(sdp.value,0) / 100  ELSE IFNULL(sdp.value,0) END AS  rate,  os.shop_name, concat('img/company/',os.image) as sellerimage,sdp.product_id,p.price,pd.name,p.image as image,
                                            'Local Product' as typedesc,'reg' as type,concat('product.php?product_id=',p.product_id,'&store_id=',sd.seller_id) as href
                                    FROM seller_deductions sd 
                                    INNER JOIN  seller_deductions_product sdp 
                                        ON sd.id=sdp.deduction_id
                                     ...
                                     INNER JOIN oc_product_to_category PTC ON PTC.product_id=psp.product_id 
                                     INNER JOIN oc_seller os
                                        ON os.seller_id=sd.seller_id
                                    WHERE sd.seller_id=:sellerid AND PTC.category_id=:category_id  AND psp.quantity!=:quantity AND p.status = :status
                                        AND 
                                            (
                                                sd.date_from <=  DATE_FORMAT(convert_tz(utc_timestamp(),'-08:00','+0:00'),'%Y-%m-%d') 
                                                and
                                    ...
                                    UNION ALL
                                    SELECT  sdp.deduction_type,sdp.value,sd.seller_id,
                                            CASE WHEN (sdp.deduction_type = 0) THEN 'Percentage (%)' ELSE 'PHP' END AS d_type,
                                            CASE WHEN (IFNULL(sdp.deduction_type,0)=0) THEN IFNULL(sdp.value,0) / 100  ELSE IFNULL(sdp.value,0) END AS  rate,  os.shop_name, concat('img/company/',os.image) as sellerimage,sdp.product_id,p.price,pd.name,p.image as image,
    ...
    public function getProductRating($product_id, $store_id, $rating){
            $result = $dataParams = array();

    $sql = "SELECT oc.review_id, oc.product_id, oc.customer_id, oc.branch_id, oc.author, oc.text, oc.rating, oc.status, DATE_FORMAT(oc.date_added,'%Y-%m-%d') as date_added
    FROM oc_review oc WHERE oc.product_id = :product_id";
    $connector = "AND";

    if (isset($store_id) && $store_id != 'all') {
      $seller_id = $store_id;
      $sql = "SELECT * FROM (SELECT oc.review_id, oc.product_id, oc.customer_id, oc.branch_id, oc.author, oc.text, oc.rating, oc.status, DATE_FORMAT(oc.date_added,'%Y-%m-%d') as date_added
      FROM oc_review oc
      INNER JOIN seller_branch sb ON sb.id = oc.branch_id AND oc.branch_id != 0
      WHERE oc.product_id = :product_id AND sb.seller_id = :seller_id
      UNION ALL
      SELECT oc.review_id, oc.product_id, oc.customer_id, oc.branch_id, oc.author, oc.text, oc.rating, oc.status, DATE_FORMAT(oc.date_added,'%Y-%m-%d') as date_added
      FROM oc_review oc
      WHERE oc.product_id = :product_id AND branch_id = 0) AS oc";
      $dataParams = array_merge($dataParams, [':seller_id' => $seller_id]);
      $connector = "WHERE";
    }

    if (isset($rating) && $rating != 'all') {
      $sql .= " $connector rating = :rating";
      $dataParams = array_merge($dataParams, [':rating' => $rating]);
    }

    $dataParams = array_merge($dataParams, [':product_id' => $product_id]);
    ...
    $rating = $this->conn->prepare("SELECT COUNT(*) AS 'all', ROUND(IFNULL(AVG(rating), 0), 1) AS 'allRating',
    (SELECT COUNT(*) FROM oc_review WHERE rating = '5' AND product_id = :product_id) as five,
    (SELECT COUNT(*) FROM oc_review WHERE rating = '4' AND product_id = :product_id) as four,
    (SELECT COUNT(*) FROM oc_review WHERE rating = '3' AND product_id = :product_id) as three,
    (SELECT COUNT(*) FROM oc_review WHERE rating = '2' AND product_id = :product_id) as two,
    (SELECT COUNT(*) FROM oc_review WHERE rating = '1' AND product_id = :product_id) as one
    FROM oc_review WHERE product_id = :product_id AND status = 1");
    $rating->execute([':product_id' => $product_id]);
    $result['rating'] = $rating->fetchObject();
    $this->totalRowCount = $stmt->rowCount();

    $ratingAverage = $result['rating']->allRating;
    $whole = floor($ratingAverage);
    $decimal = $ratingAverage - $whole;
    ...
    public function getProductReviews($product_id){
        $data = array();
        $st1 = $this->conn->prepare("SELECT oc.review_id, oc.product_id, oc.customer_id, oc.branch_id, oc.author, oc.text, oc.rating, oc.status, DATE_FORMAT(oc.date_added,'%Y-%m-%d') as date_added
    FROM oc_review oc WHERE oc.product_id = :product_id ORDER BY date_added DESC LIMIT 3");
        $st1->bindValue(':product_id', $product_id);
        $st1->execute();
        $res = $st1->fetchAll(PDO::FETCH_OBJ);
                foreach ($res as $row) {
      $data[] = array(
        ...
        'branchName' => $this->getReviewsStore($row->branch_id)->branch_name ?? "",
        'branchLogo' => $this->getReviewsStore($row->branch_id)->branch_logo ?? "",
        'sellerId' => $this->getReviewsStore($row->branch_id)->seller_id ?? "",
        'author' => utf8_encode($row->author),
        'text' => utf8_encode($row->text),
        'rating' => (int) $row->rating,
        'status' => $row->status,
        'dateAdded' => $row->date_added,
        'image' => $this->getReviewsImage($row->review_id),
      );
     // $result['reviews']['store'] = $this->getReviewsFilter($row->product_id);
    }
      return $data;
    ...
    if (isset($_GET['store']) && $_GET['store'] != 'all') {
      $seller_id = $_GET['store'];
      $sql = "SELECT * FROM (SELECT oc.review_id, oc.product_id, oc.customer_id, oc.branch_id, oc.author, oc.text, oc.rating, oc.status, DATE_FORMAT(oc.date_added,'%Y-%m-%d') as date_added
      FROM oc_review oc
      INNER JOIN seller_branch sb ON sb.id = oc.branch_id AND oc.branch_id != 0
      WHERE oc.product_id = :product_id AND sb.seller_id = :seller_id
    ...
    $data = array();
      ...
      $star = '';
      $max = 5;
      $faded = intval($row->rating);
      $notFaded = $max - $faded;
      for ($i = 0; $i < $faded; $i++) {
        $star .= '<i class="fas fa-star"></i>';
      }
      for ($i = 0; $i < $notFaded; $i++) {
        $star .= '<i class="fas fa-star not"></i>';
      }
      $data[] = array(
        'reviewId' => $row->review_id,
        'productId' => $row->product_id,
     ...
     // $result['reviews']['store'] = $this->getReviewsFilter($row->product_id);
      ...
      public function getReviewsStore($branch_id)
  {
    $stmt = $this->conn->prepare("SELECT branch_logo, b_name as branch_name, seller_id FROM seller_branch WHERE id = :branch_id LIMIT 1");
    ...
    public function getReviewsImage($review_id)
  {
    ...
    $stmt = $this->conn->prepare("SELECT `filename` FROM oc_review_image WHERE review_id = :review_id");
    $stmt->execute([':review_id' => $review_id]);

    $data = $stmt->fetchAll(PDO::FETCH_OBJ);

    foreach ($data as $row) {
      $result[] = "img/" . $row->filename;
    }

    return $result;
  }
    public function getReviewsFilter($product_id)
  {
    $stmt = $this->con->prepare("SELECT DISTINCT os.seller_id, os.shop_name FROM oc_review oc
    INNER JOIN seller_branch sb ON sb.id = oc.branch_id
    INNER JOIN oc_seller os ON os.seller_id = sb.seller_id
    WHERE oc.product_id = :product_id
    ORDER BY os.shop_name");
    $stmt->execute([':product_id' => $product_id]);

    return $stmt->fetchAll(PDO::FETCH_OBJ);
  }
  public function generateReview($product_id)
  {
    // $product_id = $_GET['product_id'];
    $firstName = $lastName = $reviews = array();
    $fullName = "";
    $reviewCount = rand(4, 9);

    $stmt = $this->conn->prepare("SELECT COUNT(*) as review_count FROM oc_review WHERE product_id = :product_id");
    $stmt->execute([':product_id' => $product_id]);

    $data = intval($stmt->fetchObject()->review_count);

    if ($data === 0) {
      for ($i = 0; $i < $reviewCount; $i++) {
        $randomRatings = rand(4, 5);

        // Generate fullname
        $stmt = $this->conn->prepare("SELECT firstname, lastname FROM oc_customer WHERE firstname NOT LIKE '%Pinoy%' AND lastname NOT LIKE '%Store%'");
        $stmt->execute();

        $data = $stmt->fetchAll(PDO::FETCH_OBJ);

        foreach ($data as $row) {
          if (!empty($row->firstname)) $firstName[] = ucwords(strtolower(trim($row->firstname)));
          if (!empty($row->lastname)) $lastName[] = ucwords(strtolower(trim($row->lastname)));
        }

        $fullName = "{$firstName[array_rand($firstName)]}  {$lastName[array_rand($lastName)]}";

        // Generate review text
        $stmt = $this->conn->prepare("SELECT `text` FROM oc_review_generate");
        $stmt->execute();
        ...
        $dateGenerated = date('Y-m-d', $date);

        $stmt = $this->conn->prepare("INSERT INTO oc_review (product_id, customer_id, branch_id, author, `text`, rating, `status`, date_added, date_modified) VALUES (:product_id, 0, 0, :author, :text, :rating, 1, :date_added, :date_modified)");
        $stmt->execute([':product_id' => $product_id, ':author' => $fullName, ':text' => $reviewText, ':rating' => $randomRatings, ':date_added' => $dateGenerated, ':date_modified' => $dateGenerated]);
      }
    }
  }
}
