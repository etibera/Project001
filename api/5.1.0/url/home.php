...
switch($source){
        ...
        case 'getDiscoverProducts':
            ...
            $data = $home->lastestPromoProducts($_GET['pageNumber'], 66);
            echo json_encode($data);
        break;
        case 'getCBProducts':
        ...
        case 'getStoreCategoryProduct':
            echo json_encode($product->storeProductCategory($_GET['pageNum'], $_GET['category_id'], $_GET['store_id']));
        break;
        case 'getStoreCategoryProductLastestPromo':
            echo json_encode($product->getSellerLatestPromoWithCategory($_GET['pageNum'], $_GET['store_id'], $_GET['category_id']));
        break;
        case 'categoryName':
            echo json_encode($category->categoryName($_GET['category_id']));
        break;
        case 'categoryById':
            echo json_encode($category->get_category_by_id($_GET['categoryId']));
        break;
        case 'checkGlobalIsAvailable':
            echo json_encode(array('valid' => true, 'errorMessage' => ''));
        break;
        case 'latestPromoName':
            echo json_encode($home->latestPromoName(66));
        break;
        case 'getSellerLatestPromo':
            echo json_encode($home->getSellerLatestPromo($_GET['sellerId'], $_GET['page']));
        break;
        case 'getGuestCategory':
            echo json_encode($home->getGuestCategory());
        break;
        case 'insertGuestCategory':
            $data = $home->insertGuestCategory($_POST['categories']);
            echo json_encode($data);
        break;
        case 'dailyDiscoverProducts':
        $data = $product->getDiscoverProducts($_GET['product_ids'], $_GET['pageNumber']);
        echo json_encode($data);
        break;
        case 'flagshipStore':
            $data = $home->flagshipStore();
            echo json_encode($data);
        break;
        case 'featuredPromo':
            $data = $home->featuredPromo($_GET['status']);
            echo json_encode($data);
        break;
        case 'brandFollow':
            $data = $home->brandFollow($_POST['customer_id'], $_POST['brand_id']);
            echo json_encode($data);
        break;
        case 'loginControlModal':
            echo json_encode(array('control' => false));
        break;
        case 'getReviews':
            echo json_encode($product->getReviews());
        break;
        default:
        echo phpinfo();
        break;
