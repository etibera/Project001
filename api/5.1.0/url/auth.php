...
switch($source){
    case 'login':
		...
		break;
		case 'landbankLogin':
		    echo json_encode($auth->landbankLogin($_POST['mobileNumber']));
		break;
		case 'fourgivesLogin':
		    echo json_encode($auth->fourgivesLogin($_POST['mobileNumber']));
		break;
		case 'landbankOnExistNumberRegister':
		    echo json_encode( $auth->landbakPayRegisterOnExistingNumber($_POST['mobileNumber'],$_POST['password']));
		break;
		case '4givesOnExistNumberRegister':
		    echo json_encode($auth->fgivesRegister($_POST['mobileNumber'],$_POST['password']));
		break;
    	default:
        break;
}
