...
class Product {
    ...
    WHERE p.status = '1' ";
        ...
        global $image;
        $sql = "SELECT pp.product_id, pp.type, pp.name, pp.price, pp.image, pp.date_added FROM (
        SELECT 'bg' as `type`, bp.product_id, bp.status, bp.product_name as name, bp.price, bp.img as image, bp.add_date as date_added FROM bg_product bp WHERE bp.status = 1
        UNION ALL ";
        $sql .= "SELECT 'reg' as `type`, p.product_id, p.status,pd.name, p.price, p.image, p.date_added ";
        $sql .= " FROM oc_product p";
  ...
  global $image;
    ...
    public function getDiscoverProducts($page_number){
        global $image;
        $product_data = array();
        $final_products = array();
        ...
        $offset = ($page_number-1) * $no_per_page;
            $s1 = $this->conn->prepare("SELECT p.image, p.product_id, p.price, pd.name FROM oc_product p 
            LEFT JOIN oc_product_description pd ON pd.product_id = p.product_id 
            WHERE p.status = '1' ORDER BY RAND() LIMIT :offset , :no_per_page");
