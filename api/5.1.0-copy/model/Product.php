...
class Product {
   ...
   public function Getfreebies($product_id,$seller_id,$branch_id){
    ...
    public function store_by_id($seller_id){
        ...
        if($data['image']){
            ...
            $img = 'https://pinoyelectronicstore.com/img/peso-seller.png';
        }
        $value = array(
            'sellerId' => $seller_id,
  ...
  global $image;
    ...
    public function getSellerLatestPromoWithCategory($pageNum, $sellerid,$category_id) {
        ...
        $offset = ($pageNum-1) * $no_per_page;
        $row=array();
        $stmt = $this->conn->prepare("SELECT sd.deduction_type,sd.value,sd.seller_id,
                                    ...
                                    UNION ALL
                                    SELECT  sd.deduction_type,sd.value,sd.seller_id,
                                            CASE WHEN (sd.deduction_type = 0) THEN 'Percentage (%)' ELSE 'PHP' END AS d_type,
                                            CASE WHEN (IFNULL(sd.deduction_type,0)=0) THEN IFNULL(sd.value,0) / 100  ELSE IFNULL(sd.value,0) END AS  rate,  os.shop_name, concat('img/company/',os.image) as sellerimage,sdp.product_id,p.price,pd.name,p.image as image,
