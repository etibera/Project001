...
class Wishlist {
    ...
    public function lists($customer_id){
        ...
        global $product;
        $data = array();
        $s = $this->conn->prepare('SELECT cw.*, pp.price, pp.name, pp.img FROM 
        (SELECT p.price, p.product_id, p.status, pd.name, p.image img FROM oc_product p LEFT JOIN oc_product_description pd ON pd.product_id = p.product_id UNION ALL SELECT bg.price, bg.product_id, bg.status, bg.product_name as `name`, bg.img FROM bg_product bg ) pp 
