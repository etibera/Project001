...
class Message {
    ...
    public function getAdminMessage($customer_id){
        $data = array();
        $stmt = $this->conn->prepare("SELECT * from(select * from (
            select IFNULL(sm.read,'') as `read`, sm.admin_id, CONCAT('PESO ADMIN') as fullname, sm.sender, sm.message, sm.timestamp , sm.id as message_id from oc_message_inbox_ca sm where sm.customer_id  = :customer_id group by sm.id order by sm.timestamp desc
            ) as views  group by views.admin_id order by views.message_id,views.timestamp desc) as views2 order by timestamp desc");
    ...
    public function getMessagesThread($sender, $receiver, $read){
        ...
        if($sender == 0){
            ...
            // $no_per_page = 12;
            // $offset = ($page_number-1) * $no_per_page;
            $stmt = $this->conn->prepare("select * from oc_message_inbox where seller_id = :sender AND customer_id = :receiver ORDER BY id DESC");
