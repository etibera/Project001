...
class Checkout {
    ...
    public function get_cart_local_perstore($customer_id,$cart_idstr){
        ...
        $select_stmt = $this->conn->prepare("SELECT os.shop_name,oc.seller_id,
                                                concat('company/',os.image) as image 
                                            FROM oc_cart oc
                                            INNER JOIN oc_seller os ON oc.seller_id = os.seller_id 
                                            WHERE oc.customer_id=:customer_id and oc.p_type=:p_type and oc.cart_id  IN (".$cart_idstr.") group by oc.seller_id,os.shop_name");
