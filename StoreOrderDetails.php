										...
										<?php foreach($order_PrdDetails as $lprd): ?>
											<tr>	
											    <td style="vertical-align: middle;text-align: center;">
											        Order-#<?php echo $lprd['order_number'];?></td>
												...
												<th colspan="4" style="vertical-align: middle;text-align: center;">
													<?php if($lprd['order_status'][0]['order_status_id']==46){ //46 sa live ?>
														<button  class="btn btn-success" title="Recieve" onclick="ReciveOrder(<?php echo $lprd['seller_id'];?>,<?php echo $_GET['order_id'];?>,'<?php echo $lprd['order_number'];?>')">
											...
											<?php foreach($lprd['details'] as $product):?>
												...
												<?php if($product['NOA']==$product['quantity']){?>
									                ...
									                <td class="text-right"><?php echo $product['price']; ?></td>
									                <?php if ($lprd['order_status'][0]['order_status_id'] == 49) {
														echo
														"<td class='position-relative'>
															<a href='review.php?order_id={$lprd['order_number']}&product_id={$product['product_id']}'>Write review here</a>
															<span class='badge bg-danger'>";
														echo intval($review->getReviewCount("orderNumber", $lprd['order_number'], $product['product_id'])) > 0 ? $review->getReviewCount("orderNumber", $lprd['order_number'], $product['product_id']) : "";
														echo "</span>
														</td>";
													} else {
														echo "<td></td>";
...
<script>
function ReciveOrder(seller_id,order_id,order_number) {
