	...
	if(isset($_POST['checkout_cart']) || isset($_GET['checkout_cart'])){
		if (isset($_POST['chkcart_id']) || isset($_GET['checkout_cart']) ){
			...
			if(isset($_POST['chkcart_id'])){
				...
				$getcart_id = array();
				$getcart_idata = explode(",", $_GET['checkout_cart']);
				if(count($getcart_idata)==1){
					array_push($getcart_id, $_GET['checkout_cart']);
				}else{
					$getcart_id=$getcart_idata;
				}
...
<div class="container">
		...
		<div class="row-home">  
				...
				if($customer_detals['telephone']=="" || $customer_detals['firstname']=="" || $customer_detals['lastname']==""){ ?>					
			        <div class="card border-0 bg-light">
			          	<div class="card-header text-center text-dark border-0" >
			          		<h2>Please update your personal information to continue</h2>
			          	</div>
			           	<div class="card-body border-0" style="background: #D3D3D3; ">  
			            	<div class="d-flex justify-content-center">
			            	<!-- 	<a href="account.php?checkout=1" class="btn btn-primary" role="button">Update now</a> -->
			            		<a id="Update_now" class="btn btn-primary" role="button">Update now</a>			            		
			            	</div>
			            	<div class="row">
			            	...
			            	<div class="d-flex justify-content-center"><a href="#" class="btn btn-primary" role="button" data-bs-toggle="modal" data-bs-target="#LoginModalexist">Log in</a>
			            	</div>
			           </div>
			        </div>  
				<?php }else{ ?>
					<div class="card border-0 bg-light">
			          	...
			          	<div class="card-body border-0" style="background: #D3D3D3; ">  
			            	...
			            	<!-- 	<a href="reg_activatemobile.php?RegIdVal=<?php echo $id;?>&checkout=1" class="btn btn-primary" role="button">Verify</a> -->
			            		<a href="" id="Verify_No" class="btn btn-primary" role="button">Verify</a>
			            	</div>
			            	<div class="row">
					          <div class="col-lg-12 text-center">
					            <div class="login-divider">OR</div>
	  	...
	  	<script type="text/javascript">
              ...
              var left  = ($(window).width()/2)-(900/2),
							...
							bottom   = ($(window).height()/2)-(600/2)                   
                  window.open("account.php?popupchk=1", "PopupWindow", "width=600,height=600,top="+top+", left="+left+",bottom="+bottom+",scrollbars=yes,resizable=no");
              }); 
              $("#Verify_No").click(function () {
              var left  = ($(window).width()/2)-(900/2),
							top   = ($(window).height()/2)-(600/2),
							bottom   = ($(window).height()/2)-(600/2)                   
                  window.open("reg_activatemobile.php?RegIdVal=<?php echo $id;?>&popupchk=1", "PopupWindow", "width=600,height=600,top="+top+", left="+left+",bottom="+bottom+",scrollbars=yes,resizable=no");
              });
          });
	...
	<?php }else if(!isset($Getdefaultaddress['region'])){?>
		<div class="row-home">  
		    <div class="row p-0">     
		      	<div class="col-sm-12 p-1"> 
		      		<div class="card border-0 bg-light">
			          	...
			          	<div class="card-body border-0" style="background: #D3D3D3; ">  
			            	<div class="d-flex justify-content-center">
			            		<!-- <a href="address_mod_update.php?aid=0&popupchk=1" class="btn btn-primary" role="button">Update Address</a> -->
			            		<a href="" id="UpdateAddress" class="btn btn-primary" role="button">Update Address</a>
			            	</div>
			            	<div class="row">
					          <div class="col-lg-12 text-center">
					            <div class="login-divider">OR</div>
	  	...
	  	<script type="text/javascript">
              ...
              var left  = ($(window).width()/2)-(900/2),
							top   = ($(window).height()/2)-(600/2),
							bottom   = ($(window).height()/2)-(600/2)                   
                  window.open("address_mod_update.php?aid=0&popupchk=1", "PopupWindow", "width=600,height=600,top="+top+", left="+left+",bottom="+bottom+",scrollbars=yes,resizable=no");
              }); 
			          	...
			          	<div class="card-header text-center text-dark border-0" >
			           	...
			           	<div class="card-body border-0 bg-light" >  
			           		<div class="d-flex justify-content-end mb-3">
			           			<div ><a href="" id="AddAddress" class="btn btn-primary" role="button" >Add Address</a></div>
			           		</div>
			           		<script type="text/javascript">
									  			$(document).ready(function () {
							 			...
							 			<table class="table table-hover table-light table-striped" >
											...
											<tr class="table-active">							
												...
												<?php foreach($localDataStores as $local_stores): ?>
													...
													<?php $insurance_feetotal+=$local_stores['insurance_fee'];?>
														<?php if($local_stores['delstats']=="Yes" && $local_stores['pickupstats']=="Yes"){ $totalflatrate+=$local_stores['flatrate']; }?>
													...
													<tr id="items-row<?php echo $items_row; ?>">
														<td colspan="2" style="vertical-align: middle;text-align: center;">
															<div class="d-flex" style="height: fit-content !important" data-toggle="tooltip" title="<?php echo $local_stores['shop_name'];?>" >
					               				<a href="product_store.php?Y2F0X2lk=<?php echo $local_stores['seller_id'];?>" target="_blank"><img clclass="img-thumbnail" src="<?php echo 'img/'.$local_stores['branch_logo'];?>" style="width: 70px;height: 70px;margin:auto" ></a>
					          					</div>															
											...
											<?php if(count($localDataStoresSmall)){?>
												<?php foreach($localDataStoresSmall as $local_storesS):  
													...
													<?php if($local_storesS['delstats']=="Yes" && $local_storesS['pickupstats']=="Yes"){ $totalflatrate+=$local_storesS['flatrate']; }?>
												  ...
												  <!-- for store orders small -->
													...
													<tr id="items-row<?php echo $items_row; ?>" style="border-top:2px solid #333;">
														<td colspan="3" style="vertical-align: middle;text-align: center;">
																<div class="d-flex" style="height: fit-content !important" data-toggle="tooltip" title="<?php echo $local_storesS['shop_name'];?>" >
					               				<a href="product_store.php?Y2F0X2lk=<?php echo $local_storesS['seller_id'];?>" target="_blank"><img clclass="img-thumbnail" src="<?php echo 'img/'.$local_storesS['branch_logo'];?>" style="width: 70px;height: 70px;margin:auto" ></a>
					          					</div>			
														</td>									
							...
							<?php if($shipping_wallet!=0){ ?> 
									<div class="mb-3 mt-3">
										<input type="checkbox" id="useSW" name="useSW" onclick="UnCheckuseSW(event,<?php echo $shipping_wallet;?>)">
											<label for="useSW" class="form-label"> Use Shipping Wallet - (<?php echo number_format($shipping_wallet,2)?> remaining)</label>
									</div>
								<?php }?>
								<!-- End For shipping_wallet -->
								<!--  For Payment Method -->
								<div class="mb-3 mt-3">
									<label for="myDropdown" class="form-label">Payment Method </label>
								<div id="myDropdown" >
						        <!-- <option value="6" data-imagesrc="https://i.imgur.com/kFAk2DX.png"
													...
													<?php if($delstats==0&&$pickupstats==0) {?> 
														<?php if($total_amount_chk<=50000) { ?> 
															<?php if($checkout->get_cart_product_cod($id,$chk_cart_id)==0) { ?> 
																<option value="<?php echo $pm['value']; ?>" data-imagesrc="<?php echo $pm['image']; ?>" data-description="<?php echo $pm['desc']; ?>"><?php echo $pm['name'];?></option>
															<?php }?>
														<?php }?>												
													<?php } ?>
								  ...
								  <label for="shipping_method" class="form-label">Shipping Method <span ><i id="ShippingMethodlabel"></i></span></label>
									<select name="shipping_method" id="shipping_method" class="form-control" onChange="update_total_shipping(this,<?php echo $shipping_wallet;?>)" required >
										...
										<?php foreach($shipMethoData as $sm):
											if($sm['disable']!=true) { ?>
												<option value="<?php echo $sm['code']; ?>" <?php if($sm['selected']){echo "selected";}?> ><?php echo $sm['name'];?></option>
										<?php } endforeach;?>
									</select>
								</div>	
...
<script type="text/javascript">
	...
	function selectshippingAdress(addr_id,cust_id,cart_ids) {
	  ...
	  $.ajax({
						...
						type: 'POST',
						data: 'address_id=' + addr_idval + '&customer_id=' + cust_id,
						dataType: 'json',
						success: function(json) {
							if (json['success']) {							
									location.reload();
	...
	$(document).delegate('#btnloginExist', 'click', function() {
		  var getcart_id='<?php echo $_GET['checkout_cart'];?>';
          ...
          success: function(json) {
            if (json['status'] == 300) {
              bootbox.alert(json['success']);
	            ...
	            location.replace("lanbankLogin.php?getcart_id="+getcart_id+"&regExistCustid=" + json['customer_id'] + "&t=" + new Date().getTime());
            }
          },
          error: function(xhr, ajaxOptions, thrownError) {
            alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
    ...
    onSelected: function (data) {
        $('#payment_method').val(data.selectedData.value);
        computetotal();
				var payment_method = data.selectedData.value;	
  	...
  	$(".dd-selected").css("text-decoration","none");
  	$(".dd-selected").css("color","#27292a");
  	$(".dd-selected").css("color","#27292a");
  	/*$("#myDropdown").css("height","250px");
