...
<div class="wrapper">
	<div class="container">
		<div class="row">
            <div class="col-xs-12">
              	<center><h2>AliExpress Products </h2></center>              
            </div>
         </div>  
		<div class="row">
		<?php										
		$list = $model_home->getProductList(1);
		if(!$list){
			?>
			...
			foreach($list as $product){ 
				$p_id_ch =$product['product_id']; 
				$goods_sn = $p_id_ch;
				$get_product_title=$product['name'];
				$name = strlen($get_product_title) > 25 ? substr($get_product_title,0,25)."..." : $get_product_title;
				$get_product_img=$product['image'];
							...
							<div class="product-thumbch transition">
								<div class="image-containerch">
									<div data-toggle="tooltip" title="Click for more details" class="imagech">
										<a  href="product_ae.php?product_id=<?php echo $p_id_ch; ?>&type=ae">
						                  <img src="<?php echo $get_product_img; ?>" alt="<?php echo $get_product_title; ?>" class="img-responsive" /></a>							                                 
									</div>
								</div>
								<div class="caption" style="text-align:center;">
					              	<h4 ><a data-toggle="tooltip" title="<?php echo $get_product_title; ?>" href="product_ae.php?product_id=<?php echo $p_id_ch; ?>&type=ae"><?php echo $name; ?></a></h4>						              		
					            </div>
