...
if (!strpos($perm, "'14';") !== false){
	...
	if(isset($_GET['att_id'])){
		$att_id=$_GET['att_id'];
		$data = $model->get_attr($att_id);
		$att_details = $model->get_attr_details($att_id);
		...
		if(isset($_POST['edit_Attribute'])){
		    ...
		    if(!isset($errorMsg) ){
		        ...
		        $res = $model->edit_attribute($_POST,$img,$a_items);
	            if($res){
...
<div id="content">
	<div class="page-header">
     <h2 class="text-center">Edit Attribute </h2>   
    </div>
    <form method="post" enctype="multipart/form-data" id="form-voucher">
    	 		...
    	 		<div class="row">
    	 			<div class="col-lg-12">
	    	 			<div class="pull-right">
					        <button type="submit" class="btn btn-primary" name="edit_Attribute" title="Save"><i data-feather="save"></i></button>
					     </div>
					 </div>
    	 		</div>
				...
				<div class="tab-content" style="margin-top: 20px">
					<div id="general" class="tab-pane fade in active">
					     ...
					     <div class="form-group required">
					     	...
					     	<input type="text" class="form-control" name="attribute_name" value="<?php echo $attribute_name;?>" required>
					     </div>
					     <div class="form-group">
					     	<label for="">Description</label>
					     	<textarea class="form-control" cols="30" rows="10" name="description" ><?php echo $description;?></textarea>
					     </div>
					</div>
					<div id="Items" class="tab-pane fade">
				         <div class="table-responsive">
				             <table id="Items_tbl" class="table table-striped table-bordered table-hover">
				                ...
				                <tbody>
				                    <?php $items_row = 0; ?>
				                    <?php   foreach($att_details as $result) { ?>    
					                    <tr id="items-row<?php echo $items_row; ?>">
					                        <td class="text-center">
					                        	<input type="text" name="attribute_item[<?php echo $items_row; ?>][name]" value="<?php echo $result['name'];?>" placeholder="Item Name" class="form-control" />
					                        </td>
					                        <td class="text-center">
					                           <input type="text" name="attribute_item[<?php echo $items_row; ?>][desc]" value="<?php echo $result['description'];?>" placeholder="Description" class="form-control" />
					                        </td>
					                        <td class="text-center">
					                        	<input type="number" name="attribute_item[<?php echo $items_row; ?>][sort_order]" value="<?php echo $result['sort_order'];?>" placeholder="Sort Order" class="form-control" />
					                        </td>
					                        <td class="text-center">
